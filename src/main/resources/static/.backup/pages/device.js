...
export class WizardPageDevice extends WizardPage {
    ...
    async showPage() {
        ...
        let response = await fetch(this._wizard.context.api("deviceClasses") + "/" + deviceClass + "/devices");
        ...
        json._embedded.devices.forEach(device => {
            ...
            if (device.imgUrl) {
                imgUrl = this._wizard.context.api("catalog") + "/" + device.imgUrl;
            }
    ...
    _gotoNextPage(device) {
        ...
        this._wizard.gotoNextPage("lend");
    }
}
